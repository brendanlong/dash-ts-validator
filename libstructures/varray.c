...
void varray_remove(varray_t* v, int i)
{
    ...
    if(i + 1 < v->length) {
        memmove(&(v->array[i]), &(v->array[i + 1]), (v->length - (i + 1)) * sizeof(varray_elem_t*));
    }
    if(v->length > 0) {
        v->length --;
    }
...
{
    varray_set(v, varray_length(v), e);
}
...
{
    varray_insert(v, varray_length(v), e);
}
...
{
    varray_elem_t* e = varray_get(v, 0);
    varray_remove(v, 0);
    return e;
}
...
{
    return varray_get_range(v, 0, varray_length(v));
}
...
varray_t* varray_get_range(varray_t* v, int start, int len)
{
    ...
    if(start + len > varray_length(v)) {
        len = varray_length(v) - start;
    }
    int i;
    for(i = 0; i < len; i++) {
...
void varray_insert_range(varray_t* v, int start, int len, varray_t* v2)
{
    ...
    if(len > varray_length(v2)) {
        len = varray_length(v2);
    }
    for(i = 0; i < len; i++) {
...
{
    varray_insert_range(v, v->length, v2->length, v2);
}
...
int varray_index_of(varray_t* v, varray_elem_t* e, int (*cmp_func)(varray_elem_t* e1,
                    varray_elem_t* e2))
{
    int i;
    for(i = 0; i < varray_length(v); i++) {
        if(cmp_func(varray_get(v, i), e)) {
            return i;
        }
    }
    return -1;
...
int varray_last_index_of(varray_t* v, varray_elem_t* e, int (*cmp_func)(varray_elem_t* e1,
                         varray_elem_t* e2))
{
    int i;
 ...
 */
int varray_binary_search(varray_t* v, varray_elem_t* e, int (*cmp_func)(varray_elem_t* e1,
                         varray_elem_t* e2))
{
...
{
    if(iter->i >= -1 && iter->i < varray_length(iter->v) - 1) {
        return 1;
    } else {
        return 0;
    }
...
{
    if(iter->i > 0 && iter->i <= varray_length(iter->v)) {
        return 1;
    } else {
        return 0;
    }
