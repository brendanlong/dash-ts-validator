...
void vqarray_insert(vqarray_t* v, int i, vqarray_elem_t* e)
{
    ...
    if(v->start > 0 && i < v->length / 2) {
        ...
        if(i > 0) {
            memmove(&(v->array[v->start - 1]), &(v->array[v->start]), i * sizeof(vqarray_elem_t*));
        }
        v->start --;
    } else {
        // move end forwards
        if(i < v->length) {
            memmove(&(v->array[i + 1 + v->start]), &(v->array[i + v->start]),
                    (v->length - i) * sizeof(vqarray_elem_t*));
        }
    }
...
void vqarray_remove(vqarray_t* v, int i)
{
    ...
    if(i < v->length / 2) {
        ...
        // move end backwards
        if(i < v->length) {
            memmove(&(v->array[i + v->start]), &(v->array[i + 1 + v->start]),
                    (v->length - (i + 1)) * sizeof(vqarray_elem_t*));
        }
    }
...
vqarray_t* vqarray_get_range(vqarray_t* v, int start, int len)
{
    ...
    if(start + len > vqarray_length(v)) {
        len = vqarray_length(v) - start;
    }
    int i;
    for(i = 0; i < len; i++) {
...
void vqarray_insert_range(vqarray_t* v, int start, int len, vqarray_t* v2)
{
    ...
    if(len > vqarray_length(v2)) {
        len = vqarray_length(v2);
    }
    for(i = 0; i < len; i++) {
...
{
    vqarray_insert_range(v, v->length, v2->length, v2);
}
                                 ...
                                 vqarray_elem_t* e2))
{
    ...
    //properly handle key less than first element of the list
    if(cmp_func(key, vqarray_get(v, lo)) < 0) {
        return -1;
    }

    //properly handle at or beyond end of array...if at or beyond end, return end
    if(cmp_func(key, vqarray_get(v, hi)) >= 0) {
        return hi;
    }
...
{
    if(iter->i >= -1 && iter->i < vqarray_length(iter->v) - 1) {
        return 1;
    } else {
        return 0;
    }
...
{
    if(iter->i > 0 && iter->i <= vqarray_length(iter->v)) {
        return 1;
    } else {
        return 0;
    }
...
{
    int memlength;
    if(v->memlength >= length + v->start) {
        return;
    }

    if(v->start > vqarray_block_length) {
        memmove(&(v->array[vqarray_block_length]), &(v->array[v->start]),
                v->length * sizeof(vqarray_elem_t*));
