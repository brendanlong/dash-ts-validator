...
void read_debug_macroblock_layer(h264_stream_t* h, bs_t* b)
{
    ...
    if(mb->mb_type == I_PCM) {
        while(!bs_byte_aligned(b)) {
            printf("%d.%d: ", b->p - b->start,
                   b->bits_left); // ERROR: value( pcm_alignment_zero_bit, f(1) ); printf("pcm_alignment_zero_bit: %d \n", pcm_alignment_zero_bit);
        }
        for(int i = 0; i < 256; i++) {
            printf("%d.%d: ", b->p - b->start, b->bits_left);
            mb->pcm_sample_luma[ i ] = bs_read_u8(b);
        ...
        for(int i = 0; i < TotalCoeff(coeff_token); i++) {
            if(i < TrailingOnes(coeff_token)) {
                ...
                if(suffixLength > 0 || level_prefix >= 14) {
                    int level_suffix;
                    printf("%d.%d: ", b->p - b->start,
                           b->bits_left); // ERROR: value( level_suffix, u ); printf("level_suffix: %d \n", level_suffix);  // FIXME
                    levelCode += level_suffix;
                }
                if(level_prefix >= 15 && suffixLength == 0) {
                    levelCode += 15;
